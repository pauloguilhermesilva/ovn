                         ...
                         const struct sset *ts_scope)
{
    ...
    {
        if (!route_ts_in_scope(ts_scope, isb_route->transit_switch)) {
            continue;
        }
 ...
 * it. */
static void
route_sync_scope(struct ic_context *ctx, const struct sset *ts_scope)
{
    ...
    delete_orphan_ic_routes(ctx, ctx->runned_az, ts_scope);
    ...
    HMAP_FOR_EACH_SAFE (ic_lr, node, &ic_lrs) {
        ...
        sync_learned_routes(ctx, ic_lr, ts_scope);
...
static void
route_lr_ts_map_init(struct ic_context *ctx, struct shash *lr_ts_map)
{
...
route_scope_finish(struct ic_context *ctx, struct sset *affected, bool updated,
                   void *node_data)
{
...
{
    struct ic_context *ctx = route_handler_ctx();
    if (!ctx) {
                                ...
                                void *data OVS_UNUSED)
{
    ...
    enum engine_input_handler_result ret =
        route_scope_finish(ctx, &affected, false, data);
                                ...
                                void *data OVS_UNUSED)
{
    ...
    route_lr_ts_map_init(ctx, &lr_ts_map);
    struct sset affected = SSET_INITIALIZER(&affected);
...
static bool
route_remove_learned_nb_route(struct engine_node *node,
                              const struct icsbrec_route *isb_route)
{
    ...
    NBREC_LOGICAL_ROUTER_TABLE_FOR_EACH (lr, lr_tbl) {
        for (size_t i = 0; i < lr->n_static_routes; i++) {
            const struct nbrec_logical_router_static_route *sr =
                lr->static_routes[i];
            const char *learned =
...
route_icsb_route_handler(struct engine_node *node, void *data OVS_UNUSED)
{
    ...
    ICSBREC_ROUTE_TABLE_FOR_EACH_TRACKED (isb_route, tbl) {
        bool ours = isb_route->availability_zone == ctx->runned_az;
        ...
        if (icsbrec_route_is_deleted(isb_route)) {
            if (ours) {
                ...
                if (route_remove_learned_nb_route(node, isb_route)) {
                    updated = true;
                }
            }
            ...
            /* A remote route changed.  Every column is routing-relevant: the
             * prefix/nexthop/origin/route_table/transit_switch/AZ, the
             * ic-route-tag in external_ids used by learn filters, and the
             * connected-route override in options.  Re-sync its TS. */
            sset_add(&affected, isb_route->transit_switch);
        } else {
    ...
    enum engine_input_handler_result ret =
        route_scope_finish(ctx, &affected, updated, data);
    sset_destroy(&affected);
    return ret;
}
