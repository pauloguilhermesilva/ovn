...
static bool
    route_need_learn(const struct route_input *route_input,
                 ...
                 struct in6_addr *nexthop)
{
    ...
    const struct sbrec_datapath_binding *dp =
        find_sb_dp_by_nb_uuid(route_input->sbrec_datapath_binding_by_nb_uuid,
                              &lr->header_.uuid);
    if (!dp) {
        return true;
    }
    ...
    struct sbrec_learned_route *filter = sbrec_learned_route_index_init_row(
        route_input->sbrec_learned_route_by_datapath);
